...
If nOpc == 1 // Inclusao
    If aN == 1 // Só efetiva na primeira parcela
    RecLock("SE2",.F.)
    SE2->E2_CODBAR := __cCamNovo
    SE2->E2_LINDIG := FinCBLD(__cCamNovo) // FinCBLD é responsável por transformar o código de barras em linha digitável
    EndIf
MsUnLock()
Endif
Return()
